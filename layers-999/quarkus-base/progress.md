...
### Efforts to build-time initialize ApplicationImpl in base layer

With `ApplicationImpl` initialized at build time, its `<clinit>` runs the full Quarkus startup sequence. This pulls in a deep chain of dependencies that must all be present in `extracted-classes.jar`. The `split-jar.sh` script was extended to extract these, revealing the dependency chain one error at a time.

**Classes/patterns needed by ApplicationImpl build-time init (in order of discovery):**

1. **`$$CMImpl` classes** — SmallRye ConfigMapping implementations (79 classes). Generated by Quarkus augmentation. Without them: `SRCFG00051: Could not generate ConfigMapping ... NoClassDefFoundError: org/objectweb/asm/ClassVisitor` (SmallRye tries to generate them dynamically using ASM, which isn't available in native-image).

2. **`io/quarkus/runner/recorded/*`** — Quarkus recorder classes (52 classes). Generated build step implementations. Without them: `NoClassDefFoundError: io/quarkus/runner/recorded/ResteasyReactiveCommonProcessor$setupBlockingOperationSupport688479453`.

3. **`META-INF/services`** — ServiceLoader metadata. Without it: `Annotation is not a registered qualifier: interface jakarta.enterprise.inject.Any` because `ServiceLoader.load(ComponentsProvider.class)` returns no implementations.

4. **`*_ComponentsProvider*`** — Arc CDI component provider classes. The actual ServiceLoader implementation for CDI bean registration.

5. **`*_Bean`** — Arc-generated CDI bean classes (26 classes). Without them: `NoClassDefFoundError: io/quarkus/vertx/runtime/VertxProducer_Bean`.

6. **`*_ClientProxy`** — Arc-generated CDI client proxy classes (8 classes). Without them: `NoClassDefFoundError: io/quarkus/vertx/runtime/VertxProducer_ClientProxy`.

7. **`*_ArcAnnotationLiteral`** — Arc-generated annotation literal classes (8 classes).

8. **`*_ContextInstances`** — Arc-generated context instance classes (2 classes).

9. **`org/acme/*`** — Application classes. Arc beans reference the actual application classes.

10. **Library classes from `lib/*`** — After extracting all the above, the init chain reaches into library JARs (e.g. `io.smallrye.context.SmallRyeManagedExecutor`). These are already on the classpath via `lib/*` but the full Quarkus startup keeps pulling in more transitive dependencies.

**Gotchas discovered:**

- **Dot vs slash in class paths:** Class entries in `split-jar.sh` MUST use `/` separators, not `.`. Using dots causes `dirname` to return `.` instead of the proper directory path, and the class file won't be found.

- **`$$` escaping in bash arrays:** Inner class names with `$$` (like `Config$$CMImpl`) need `\$\$` escaping in bash array entries.

- **CMImpl classes can't be generated at build time:** SmallRye Config normally generates `$$CMImpl` classes dynamically using ASM bytecode generation. In native-image, ASM is not on the classpath, so these classes must be pre-generated by Quarkus augmentation and present in the JAR.

- **ServiceLoader metadata placement matters:** The `META-INF/services` files must be in the same JAR as the implementation classes. Moving `_ComponentsProvider` classes to `extracted-classes.jar` without the corresponding service file means `ServiceLoader.load()` can't find them.

- **ApplicationImpl build-time init is a deep rabbit hole:** The Quarkus startup sequence triggered by `ApplicationImpl.<clinit>` transitively needs virtually every class in the application — config classes, recorder classes, CDI beans, client proxies, application code, and library classes. The split-jar approach hits diminishing returns as the dependency chain extends into `lib/*` JARs.

**Status:** BLOCKED — After extracting all runner JAR classes, the init chain reaches library classes (`SmallRyeManagedExecutor`). The fundamental issue is that build-time init of `ApplicationImpl` runs the full Quarkus startup, requiring nearly all classes to be available. A different approach may be needed (e.g. runtime init of `ApplicationImpl`, or restructuring what goes in the base layer).
