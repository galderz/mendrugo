...
perf stat ${OPT}/${ID}/bin/native-image ${ARGS}

echo "# Profile with async profiler"
${JAVA_HOME}/bin/native-image -J-agentpath:${OPT}/async-profiler/lib/libasyncProfiler.so=start,event=${EVENT},file=${EVENT}-${ID}.${FORMAT} ${ARGS}
